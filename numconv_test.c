...
void run_tests()
{
    ...
    wrongs_corrects[run_from_int_tests(-1073626914, "FFFF FFFF C001 C0DE", int_to_hex, "hex")]++;
...
int run_from_int_tests(int val_in_int, char *expected, void (*conv_f)(int, char *), char *num_type)
{
    ...
    {
        printf("Test FAILED for %s. Expected: %s. Actual: %s. (int=%d) \n", num_type, expected, buffer, val_in_int);
        return 0;
    }
    return 1;
}